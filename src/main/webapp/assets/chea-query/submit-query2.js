...
function intersectionPopover(row, library) {
	var genes = row.Overlapping_Genes.split(','),
			genes_link = genes.map(function(x) { return `<a href="https://maayanlab.cloud/Harmonizome/gene/${x}" target="_blank">${x}</a>` });
	return `
	...
	if (library.includes('Integrated')) {
		...
		$table.DataTable({
			...
			columns: [
				...
				{ "mData": "TF", "sTitle": "TF", "mRender": function (x) { return `<a href="https://maayanlab.cloud/Harmonizome/gene/${x}" target="_blank">${x}</a>` } , "className": "dt-head-center"},
